...
def play_inference():
    print(f"[START] task=smart_grid_tasks env=smart_grid_optimizer model={MODEL_NAME}", flush=True)
...
"""
    ...
    while not done:
        ...
        action_str = f"{action_type}_{amount}"
        done_str = str(data['done']).lower()
        print(f"[STEP] step={step_id} action={action_str} reward={data['reward']:.2f} done={done_str} error=null", flush=True)
